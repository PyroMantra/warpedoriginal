...
# --- Minimal chat.js if missing (your existing chat.js will remain; we only create if absent) ---
...
# --- Template ops: canonical panel, cache-busted links, brand→home, and fix stray "1{{ ... }}" ---
if not tpl_dir.exists():
    raise SystemExit("No templates/ directory found.")

def remove_existing_panels(html: str) -> str:
    return re.sub(r'<div[^>]*id\s*=\s*"global-chat"[\s\S]*?</div>', '', html, flags=re.I)

def ensure_head_css(html: str) -> str:
    # add/replace chat.css with ?v=STAMP
    ...
    if re.search(r"</head\s*>", html, re.I):
        html = re.sub(r"</head\s*>", lambda m: link_tag + "\n</head>", html, count=1, flags=re.I)
    else:
        html = link_tag + "\n" + html
    return html
...
def ensure_footer_scripts(html: str) -> str:
    ...
    if "chat.js" in html:
        ...
        inject = f'<script>window.CURRENT_USER_NAME = "{{{{ current_user_name }}}}";</script>\n<script src="{{{{ url_for(\'static\', filename=\'chat.js\') }}}}?v={STAMP}"></script>'
        html = re.sub(r"</body\s*>", lambda m: inject + "\n</body>", html, count=1, flags=re.I) or (html + "\n" + inject + "\n")
    return html

def ensure_brand_goes_home(html: str) -> str:
    # Convert any anchor with inner text matching "Across the Planes" to href="{{ url_for('home') }}"
    def anchor_repl(m):
        attrs = m.group("attrs") or ""
        inner = m.group("inner")
        attrs = re.sub(r'\s+href\s*=\s*(["\']).*?\1', '', attrs, flags=re.I)  # strip existing href
        new_attrs = f' href="{{{{ url_for(\'home\') }}}}"' + attrs
        return f"<a{new_attrs}>{inner}</a>"

    html2 = re.sub(r"<a(?P<attrs>[^>]*)>(?P<inner>.*?Across\s*the\s*Planes.*?)</a>",
                   anchor_repl, html, flags=re.I|re.S)

    # JS fallback (insert via callable to avoid backslash escape issues)
...
document.addEventListener('DOMContentLoaded', function(){
  document.querySelectorAll('a, .brand, .navbar-brand, header *, [data-brand]').forEach(function(el){
    ...
    if 'data-brand-fallback="1"' not in html2:
        if re.search(r"</body\s*>", html2, re.I):
            html2 = re.sub(r"</body\s*>", lambda m: fallback + "\n</body>", html2, count=1, flags=re.I)
        else:
            html2 = html2 + "\n" + fallback + "\n"
    return html2
...
def inject_panel(html: str) -> str:
    ...
    if re.search(r"</body\s*>", cleaned, re.I):
        return re.sub(r"</body\s*>", lambda m: CANON_PANEL + "\n</body>", cleaned, count=1, flags=re.I)
    return cleaned + "\n" + CANON_PANEL + "\n"
...
for html_file in sorted(tpl_dir.rglob("*.html")):
    ...
    # 1) Fix lingering 1{{ current_user_name }} or \1{{ ... }}
